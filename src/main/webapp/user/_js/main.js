...
document.addEventListener("DOMContentLoaded", () =>{
        ...
        api.getStats(fromDate,toDate)
            .then(
                (result)=>{
                    ...
                    console.log(result.map((s)=> s.categoryShares))
                }
            )
            .catch((err) => {
    ...
    function setStats(result){
        ...
        paymentCount.innerText=result.map((s)=>s.allPayments);

        
        allAmount.innerText=result.map((s)=>s.allAmount);

        

        const suppliers=result.map((s)=>s.topSuppliers);
    

        let suppliersText="";
        if(suppliers.length>0){
            for(let i=0;i<suppliers[0].length;i++){
                //console.log(suppliers[0][i].title);
                suppliersText+=suppliers[0][i].title+" ";
        ...
        let methods=[];
        methods=result.map((s)=>s.paymentMethodShares);

        for(let i=0;i<methods.length; i++){
            for(let j=0;j<methods[i].length;j++){
                //console.log(methods[i][j]);
                paymentMethodShares.append(setPaymentMethodShare(methods[i][j]));
            } 
        }
        let categories=result.map((s)=> s.categoryShares)
    ...
    function setPaymentMethodShare(result){
        ...
        methodText.innerText=result.method.title;
    ...
    function setCategoryShares(result,block){
        ...
        for(let i=0;i<count;i++){
            ...
            const height=(category.percent*2)+"px";
            const width=(window.innerWidth/count/2)+"px";
            columnColumn.setAttribute("style","height: "+height+"; background-color:"+category.category.color);
            ...
            columnTitle.innerText=category.category.title+" "+category.amount;
